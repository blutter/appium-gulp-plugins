    ...
    gulp.task(taskName, function () {
      return exec('rm -rf ./build').then(function () {
        return globby(covTestFiles).then(function (files) {
          let appiumGulpPath = path.join(require.resolve("appium-gulp-plugins"), "..");
          console.log(`\nappiumGulpPath ${appiumGulpPath}`);
          let babelHookPath = path.join(appiumGulpPath, "babelhook");

          let babelPath = path.normalize(require.resolve("babel"));
          console.log(`\n0. ${globby.sync(babelPath, ["**/node_modules"])}`);
          // strip first node_modules.*
          let topLevelNodeModulesSubStr = babelPath.match(new RegExp(/(.*?)(node_modules)/gi));
          let topLevelNodeModulesPath = topLevelNodeModulesSubStr[0];

          var deferred = Q.defer();
          var bins = {};
          _.each(['isparta', 'babel-node', '_mocha'], binName => {
             let binPath = globby.sync(path.join(topLevelNodeModulesPath, `/.bin/${binName}`))[0]; 
             console.log(binPath);
             bins[binName] = path.normalize(binPath);
          });

          console.log(`\nmyBins = ${JSON.stringify(bins)}`);

          //function (k) { bins[k] = path.resolve(__dirname, '../node_modules/.bin', k); });
          var bin = bins['babel-node'];
          var args = [bins.isparta, 'cover', '--dir', targetDir, bins._mocha, '--', '--require', babelHookPath, '--reporter', 'dot']
            .concat(files);
